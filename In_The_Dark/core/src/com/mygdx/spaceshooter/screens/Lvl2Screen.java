...
public class Lvl2Screen implements Screen {
    InTheDark game;
    Lvl3Screen lvl3Screen;
    ...
    public Lvl2Screen (InTheDark gam) {
    ...
    private void actions() {
       ...
       // if (TimeUtils.millis() - lastenemyNinjaSpawnTime > enemyNinjaSpawnInteval && ninjas.size < 3 && !bossInGame)
        ...
        if(boss != null && (TimeUtils.millis() - boss.getLastShootTime() > bossShootInterval) && !boss.isInAttack()) {
        ...
        if (score >= 100 && !bossInGame) {
            ninjas.clear();
            enemyArchers.clear();
            enemyWarriors.clear();
            boss = new Boss2(trees,player,animationBossAttack,animationBossAttack2,animationBossMove,animationBossMove2,animationBossShoot,animationBossShoot2);
            bossInGame = true;
        }
    ...
    void spawnBossShoot(){
       ...
       enemyArchers.add(new EnemyArcher(new Animation(trAnimArcher1,6,0.5f), new Animation(trAnimArcher2,6,0.5f),trees));
        ...
        for (int i = 0; i < enemyArchers.size; i++) {
            enemyArchers.get(i).setWidth(GameScreen.SCR_WIDTH/40f);
            enemyArchers.get(i).setHeight(GameScreen.SCR_HEIGHT/20f);
            enemyArchers.get(i).setX(MathUtils.random(0, SCR_WIDTH));
            enemyArchers.get(i).setY(MathUtils.random(0, SCR_HEIGHT));
        }
    ...
    private void loadResources(){
        ...
        bg1 = new Texture("PaternForLvl2_1_v1.png");
        bg2 = new Texture("PaternForLvl2_2_v1.png");
        bg3 = new Texture("PaternForLvl2_3_v1.png");
    ...
    public void render(float delta) {
        ...
        if(score <= 100) {
            font.draw(batch, score + "/100", SCR_WIDTH / 12f, SCR_HEIGHT - 10);
        }else{
            font.draw(batch,  "100/100", SCR_WIDTH / 12f, SCR_HEIGHT - 10);
        }

        if(boss != null)
            font.draw(batch,boss.getHp() + "/5000",boss.getX() - boss.getWidth()/2f,boss.getY() + boss.getHeight() + 10);
        ...
        if(blinkInterval - (TimeUtils.millis()-lastBlinkTime) > 0 && player.isAlive())
            font.draw(batch, "COOLDOWN BLINK: " + Math.round((blinkInterval - (TimeUtils.millis()-lastBlinkTime))/1000f), SCR_WIDTH - SCR_WIDTH/5f - 150, SCR_HEIGHT - 10);
